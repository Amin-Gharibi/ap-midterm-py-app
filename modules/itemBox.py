...
class ItemBox(tk.Frame):
    def __init__(self, master, target_fg_color, details_page, item, *args, **kwargs):
        ...
        if 'description' in self.item.keys() and len(self.item['description']):
            description_label_height = 80
        else:
            description_label_height = 0

        # description label
        description_label = ctk.CTkLabel(self, text=self.item['description' if 'description' in self.item.keys() else 'biography'], fg_color=target_fg_color[1], font=("Arial", 12), text_color='gray', height=description_label_height)
